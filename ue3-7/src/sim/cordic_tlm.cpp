...
Cordic_TLM::Cordic_TLM(sc_module_name name)
    : sc_module(name), mSocket("cordic_tlm_rw"), mClk("clk", CLK_PERIOD_NS, SC_NS) {
    ...
    sensitive << mBhvRdy << mBhvX << mBhvY << mCcRdy << mCcX << mCcY << mPhi << mStartCalculations;
...
void Cordic_TLM::combineAndCheckSignals() {
    ...
    if (bhvFinished && ccFinished) {
        ...
        // Check if the difference between the results is smaller than the given epsilon.
        if (std::abs(mBhvX - mCcX) > EPSILON) {
            sc_assertion_failed("The difference in X is greater than epsilon", __FILE__, __LINE__);
        }
        if (std::abs(mBhvY - mCcY) > EPSILON) {
            sc_assertion_failed("The difference in Y is greater than epsilon", __FILE__, __LINE__);
        }
