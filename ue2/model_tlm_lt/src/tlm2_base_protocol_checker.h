...
class tlm2_base_protocol_checker
 ...
 private:
  ...
  void tlm2error(tlm::tlm_generic_payload& trans,
                 const char* ref,
                 bool warning = false);
...
nb_transport_fw_post_checks(tlm::tlm_generic_payload& trans,
  ...
  if (status == tlm::TLM_UPDATED) {
    nb_transport_response_checks(trans,
                                 phase,
                                 delay,
                                 "(forward) return",
  ...
  switch (phase) {
    ...
    case tlm::END_REQ:
      if (m_map[&trans].ph != tlm::BEGIN_REQ) {
        txt << "Phase " << phase << " sent out-of-sequence on " << txt2
            << " path"
            << ", detected in " << txt4;
        tlm2error(trans, "15.2.4");
      }
...
check_response_path(tlm::tlm_generic_payload& trans, const char* txt2) {
  if (!shared_map[&trans].path.empty()) {
    ...
    if (!shared_map[&trans].resp_data_ptr) {
      ...
      memcpy(shared_map[&trans].resp_data_ptr,
             trans.get_data_ptr(),
