...
Cordic_TLM::Cordic_TLM(sc_module_name name)
  ...
  sensitive << mBhvRdy << mBhvX << mBhvY << mCcRdy << mCcX << mCcY << mPhi
            << mStartCalculations;
...
void Cordic_TLM::combineAndCheckSignals() {
  constexpr double EPSILON = std::pow(2, -15);

  static bool bhvFinished = false;
  static bool ccFinished = false;

  // Rising edges of a ready signal indicates that the calculation has finished
  if (mBhvRdy.posedge()) {
    mBhvStart = false;
    bhvFinished = true;
  }
  ...
  // When mStartCalculations is toggled, new calculations have been started
  ...
  // The whole calculation is finished when both IPs finished their
  // calculations.
  ...
  if (bhvFinished && ccFinished) {
    ...
    // Check if the difference between the results is smaller than the given
    // epsilon.
    if (std::abs(mBhvX - mCcX) > EPSILON) {
      sc_assertion_failed(
          "The difference in X is greater than epsilon", __FILE__, __LINE__);
    }
    if (std::abs(mBhvY - mCcY) > EPSILON) {
      sc_assertion_failed(
          "The difference in Y is greater than epsilon", __FILE__, __LINE__);
    }
...
void Cordic_TLM::b_transport(tlm::tlm_generic_payload &trans, sc_time &delay) {
  ...
  if (tr_addr != OFFSET_CTL && tr_addr != OFFSET_PHI && tr_addr != OFFSET_XY) {
  ...
  if (tr_cmd == tlm::TLM_READ_COMMAND) {
    ...
    switch (tr_addr) {
      ...
      default:
        SC_REPORT_ERROR(
            "TLM-2",
            "Cordic_TLM: Invalid register address used in read access");
        return;
    }
    ...
    if (tr_addr == OFFSET_PHI) {
      ...
      // Convert interger representation back to a double and eventually into
      // sc_ufixed
      ...
      SC_REPORT_ERROR(
          "TLM-2", "Cordic_TLM: Invalid register address used in write access");
      return;
    }
  }
