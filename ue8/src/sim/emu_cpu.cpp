...
EmuCpu *EmuCpu::createInstance(char const *module_name,
                               main_func_ptr_t main_entry_point) {
  if (mInstance) {
...
void EmuCpu::ISR() {
  if (mIsrFunc == NULL)
    cerr << "EmuCpu::ISR() was called, before the ISR callback was set!"
         << endl;
  else
    mIsrFunc(NULL);
}
...
void EmuCpu::doTransaction(tlm::tlm_command cmd,
                           uint32_t addr,
                           uint32_t *data) {
